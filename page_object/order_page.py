...
class OrderPage(BasePage):
    def set_name(self, name):
        self.find_element_located(OrderPageLocators.NAME_FORM).send_keys(name)

    def set_surname(self, surname):
        self.find_element_located(OrderPageLocators.SURNAME_FORM).send_keys(surname)

    def set_address(self, address):
        self.find_element_located(OrderPageLocators.ADDRESS_FORM).send_keys(address)

    def select_metro_station(self):
        self.find_element_located(OrderPageLocators.METRO_STATION_SELECTOR).click()

    def set_phone_number(self, number):
        self.find_element_located(OrderPageLocators.PHONE_NUMBER_FORM).send_keys(number)

    def click_next_button(self):
        self.find_element_located(OrderPageLocators.NEXT_BUTTON).click()

    def select_delivery_day(self):
        self.find_element_located(OrderPageLocators.DELIVERY_SCOOTER_DAY_FORM).click()

    def select_rental_period(self):
        self.find_element_located(OrderPageLocators.RENTAL_PERIOD_FORM).click()

    def select_scooter_color(self, locator):
        self.find_element_located(locator).click()

    def pass_a_comment_for_courier(self, comment):
        self.find_element_located(OrderPageLocators.COURIER_COMMENTS_FORM).send_keys(comment)

    def click_order_button(self):
        self.find_element_located(OrderPageLocators.ORDER_BUTTON).click()

    def confirm_order(self):
        self.find_element_located(OrderPageLocators.CONFIRMATION_ORDER_BUTTON).click()
